...
SWITCH_VALVE6 = "switch.dev4_gpio_15"
...
#		self.run_daily(self.start_o, datetime.time(18, 8, 0))
	...
	def start(self, entity, attribute, old, new,kwargs):
		...
		self.config = [
			[[SWITCH_VALVE6],1.5], # one valve, 4x t200 frontlawn, 150%
	...
	def sm(self, entity="", attribute="", old="", new="",kwargs=""):
		...
		if(int(self.state/1000) == 4):
			# shut power down
			self.set_state("sensor.dev30_state",state="Shutting down 1")
			self.state = 5000 # finish 2
			self.log("Shutting down pump")
			self.turn_off(SWITCH_PUMP)
			self.run_in(self.sm,5) # call in again in 5 sec

		########## finish up 2/2
		if(int(self.state/1000) == 5):
			self.set_state("sensor.dev30_state",state="Shutting down 2")
